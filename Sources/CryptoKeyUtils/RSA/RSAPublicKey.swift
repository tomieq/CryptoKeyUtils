    ...
    public init(pem: String) throws {
        var format: RSAPublicKeyFormat {
            get throws {
                ...
                throw RSAPublicKeyError.invalidPemStructure(reason: "Unknown PEM private key header or footer")
            }
        }
        let pemFormat = try format
        print("Detected Public RSA Key PEM in format \(pemFormat)")
        let rawPem = pem
        ...
        switch pemFormat {
        case .pkcs1:
            try self.init(pkcs1: try der.asn1)
        case .subjectPublicKeyInfo:
            try self.init(subjectPublicKeyInfo: try der.asn1)
        }
    }
